...
parse inp = sch
  where
    (_, sch) = L.foldl' (\(i, sch) r ->
                           let (_, sch', js, cur) = L.foldl' (row i) (0, sch, [], 0) r
    ...
    row i (j, sch, js, cur) c =
      if C.isDigit c then (j+1, sch, j:js, cur * 10 + C.digitToInt c)
...
nextToSymbol sch = Set.fromList nexts
  where nexts = [ n | (s, Symbol _) <- Map.assocs sch
                    , k <- neighbours s
...
gearRatios sch = ratios
  where ratios = [ product [ n | Number n _ <- toList gs]
                 | (s, Symbol '*') <- Map.assocs sch
