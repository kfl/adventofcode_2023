...
step (nr, nc, mirrors) (beams, energized) = (beams', Set.union beams' energized)
  where
    beams' = Set.difference (unionsMap next beams) energized
    next b@(c,_) = filter inBounds bs
      where bs = Map.findWithDefault continue c mirrors b
            inBounds ((x,y),_) = 0 <= x && x < nc && 0 <= y && y < nr

energize input start = Set.size $ Set.map fst energized
  where steps = iterate (step input) (Set.singleton start, Set.empty)
        energized = maybe Set.empty snd $ L.find (Set.null . fst) steps

part1 :: Input -> Int
part1 input = energize input ((-1, 0), East)
answer1 = part1 <$> input
...
part2 input@(nr, nc, _) = maximum potential
  where potential = runPar $ parMap (energize input) $ startPositions nr nc
answer2 = part2 <$> input
